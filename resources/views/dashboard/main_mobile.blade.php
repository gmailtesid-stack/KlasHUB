...
<body class="antialiased selection:bg-brand-500/30 flex h-screen overflow-hidden text-sm relative">
    ...
    <div x-data="{ 
        ...
        editJabatanTarget: null,
        editJabatanRole: '',
        openEditJabatan(mhs) {
            this.editJabatanTarget = mhs;
            this.editJabatanRole = mhs.role;
            this.modalEditJabatan = true;
        },
        submitEditJabatan() {
            if (!this.editJabatanRole || this.editJabatanRole === this.editJabatanTarget.role) {
                this.modalEditJabatan = false;
                return;
            }
            fetch('/kh/student/' + this.editJabatanTarget.id + '/role', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').getAttribute('content')
                },
                body: JSON.stringify({ role: this.editJabatanRole })
            }).then(res => res.json()).then(res => {
                if(res.success) {
                    alert('Berhasil ubah jabatan!\n\nPassword login baru untuk ' + this.editJabatanTarget.name + ' sekarang menjadi: ' + res.default_password);
                    location.reload();
                } else {
        ...
        matkuls: [
            @foreach($master_subjects as $ms)
                {
                    id: {{ $ms->id }},
                    name: {{ json_encode($ms->name) }},
                    sks: {{ $ms->sks }},
                    code: '{{ $ms->code }}',
                    lecturer: {{ json_encode($ms->default_lecturer) }}
                },
            @endforeach
        ],
        get matkuls_sks() {
        ...
        jadwalHarian: [
            @foreach($jadwal_harian as $jd)
                {
                    id: {{ $jd->id }},
                    matkul: {{ json_encode($jd->subject_name) }},
                    dosen: {{ json_encode($jd->lecturer_name) }},
                    hari: '{{ $jd->day }}',
            ...
            .then(data => {
                if(data.success) {
                    ...
                    if(item) {
                        ...
                        if(type === 'cash') {
                            if(item.type === 'income') {
                                this.saldoKas += parseInt(item.amount);
                                this.pemasukanMingguIni += parseInt(item.amount);
                            } else {
                                this.saldoKas -= parseInt(item.amount);
                                this.pengeluaranMingguIni += parseInt(item.amount);
                            }
                        }
                    }
        ...
        init() {
            ...
            fetch('/kh/api/dashboard-data')
                ...
                .then(data => {
                    ...
                    this.semuaTugas = data.semua_tugas.map(t => ({...t, isValidated: t.is_validated}));
                    this.semuaModul = data.semua_modul.map(m => ({...m, isValidated: m.is_validated}));
                    ...
                    this.qrisImage = data.qris_image || '';
    ...
    }" x-init="init()" @notify-toast.window="notify($event.detail)" class="flex-1 flex min-w-0 w-full h-full relative">
            ...
            <nav class="flex-1 p-5 space-y-3 overflow-y-auto">
                <form method="GET" action="{{ url()->current() }}" class="mb-4 pb-4 border-b border-zinc-900/50">
                    ...
                    <select name="semester" onchange="this.form.submit()"
                        class="w-full bg-zinc-900/50 border border-zinc-800 text-zinc-300 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-2.5 outline-none hover:border-zinc-700 transition appearance-none text-center font-bold">
                        @for($i = 1; $i <= ($class_semester ?? 1); $i++)
                            <option value="{{ $i }}" {{ request('semester', $class_semester ?? 1) == $i ? 'selected' : '' }}>
                                Data Semester {{ $i }}</option>
                        @endfor
                    </select>
                </form>
                ...
                @if (in_array($student->role ?? '', ['ketua_kelas', 'sekretaris', 'bendahara', 'super_admin']))
                    <button @click="tab = 'admin'"
                        :class="tab === 'admin' ? 'bg-amber-600/20 text-amber-500 border-amber-500/30' : 'text-zinc-400 hover:bg-zinc-900/50 hover:text-zinc-200 border-transparent'"
                        class="w-full flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all border text-left font-medium">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="flex-1 flex flex-col h-full min-w-0 bg-black relative">
            ...
            <header
                ...
                <div class="flex items-center gap-4">
                    ...
                    <form method="GET" action="{{ url()->current() }}">
                        <select name="semester" onchange="this.form.submit()"
                            class="bg-zinc-900 border border-zinc-800 text-[10px] font-bold rounded-lg text-zinc-300 px-2 py-1.5 focus:ring-blue-500 focus:border-blue-500 max-w-[90px] outline-none">
                            @for($i = 1; $i <= ($class_semester ?? 1); $i++)
                                <option value="{{ $i }}" {{ request('semester', $class_semester ?? 1) == $i ? 'selected' : '' }}>Smstr {{ $i }}</option>
                            @endfor
                        </select>
                    </form>
                    <form method="POST" action="/logout">
                ...
                <div class="max-w-5xl mx-auto w-full h-full">
                            ...
                            <template x-for="n in notifications" :key="n.id">
                                <div @click="markAsRead(n.id)"
                                    ...
                                    <div class="flex gap-4 items-start relative z-10">
                                        ...
                                        <div class="flex-1">
                                            <p class="text-sm text-zinc-100 font-medium leading-relaxed mb-1"
                                                x-text="n.message"></p>
                                            <p class="text-[10px] text-zinc-500 font-bold uppercase tracking-widest"
                                                x-text="new Date(n.created_at).toLocaleString('id-ID', {day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit'})">
                                            </p>
                                        </div>
                                        <template x-if="!n.is_read">
                                            <span
                                                class="w-2.5 h-2.5 bg-blue-500 rounded-full animate-pulse shadow-lg shadow-blue-500/50"></span>
                                        </template>
                                    </div>
                                </div>
                            </template>
                                        ...
                                        <thead>
                                            <tr class="border-b border-zinc-900">
                                                ...
                                                <th
                                                    ...
                                                    Anggota</th>
                                                <th
                                                    class="text-right px-6 py-3 text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                                                    Aksi</th>
                                            </tr>
                                        </thead>
                    ...
                    @if(in_array($student->role ?? '', ['ketua_kelas', 'sekretaris', 'bendahara', 'super_admin']))
                        <div x-show="tab === 'admin'" x-transition:enter="transition ease-out duration-300"
                            x-transition:enter-start="opacity-0 translate-y-4"
                            x-transition:enter-end="opacity-100 translate-y-0" style="display: none;">
                            <h2 class="text-2xl md:text-3xl font-bold text-white tracking-tight mb-8">Panel Manajemen
                                Pengurus</h2>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @if(in_array($student->role ?? '', ['ketua_kelas', 'super_admin']))
                                    <!-- NAIK SEMESTER -->
                                    <div
                                        class="bg-red-950/20 border border-red-900/30 p-5 rounded-2xl flex flex-col justify-between">
                                        <div>
                                            <h3 class="text-red-500 font-bold mb-2">Promosi Semester Baru</h3>
                                            <p class="text-zinc-500 text-xs mb-4">Mengarsipkan seluruh data akademik dan
                                                keuangan, mengatur kelas Anda menjadi kertas kosong untuk semester baru.</p>
                                        </div>
                                        <button
                                            @click="if(confirm('Data semester ini tidak akan terhapus namun diarsipkan. Lanjutkan Promosi Semester?')) { fetch('/kh/class/next-semester', {method: 'POST', headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}}).then(()=>location.reload()); }"
                                            class="bg-red-600/80 text-white font-bold w-full py-2.5 rounded-xl hover:bg-red-600 transition outline-none">Naik
                                            Semester Sekarang</button>
                                    </div>

                                @else
                                    <div
                                        class="bg-blue-950/20 border border-blue-900/30 p-5 rounded-2xl col-span-1 md:col-span-2">
                                        <p class="text-blue-500 text-sm font-bold text-center">Hak Akses Terbatas. Hanya Ketua
                                            Kelas yang berhak mengakses Mutasi Mahasiswa dan Promosi Semester.</p>
                                    </div>
                                @endif
                                ...
                                @if(in_array($student->role ?? '', ['ketua_kelas', 'bendahara', 'super_admin']))
                                    <!-- UPLOAD QRIS KAS -->
                                    <div
                                        class="bg-emerald-950/20 border border-emerald-900/30 p-5 rounded-2xl flex flex-col justify-between">
                                        <div>
                                            <h3 class="text-emerald-500 font-bold mb-2">Manajemen QRIS Kelas</h3>
                                            <p class="text-zinc-500 text-xs mb-4">Unggah / perbarui bar-code QRIS statis untuk
                                                memudahkan pembayaran iuran kas.</p>
                                        </div>
                                        <button @click="modalUploadQris = true"
                                            class="bg-emerald-600/80 text-white font-bold w-full py-2.5 rounded-xl hover:bg-emerald-600 transition outline-none shadow flex items-center justify-center gap-2"><svg
                                                class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M12 4v16m8-8H4"></path>
                                            </svg> Upload Kode QRIS Baru</button>
                                    </div>
                                @endif
                            </div>
                        </div>
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    <div class="flex items-center gap-3 mt-2">
                                                        ...
                                                        @if(in_array(($student->role ?? ''), ['ketua_kelas', 'super_admin']))
                                                            ...
                                                            <button
                                                                @click="if(confirm('Hapus jadwal ini?')) { fetch('/kh/schedule/'+jdwl.id, {method: 'DELETE', headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}}).then(()=>location.reload()); }"
                                                                class="bg-red-600/20 border border-red-500/30 text-red-500 text-[8px] px-2 py-0.5 rounded font-bold hover:bg-red-600 hover:text-white transition ml-1 cursor-pointer">Hapus</button>
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div @click="selectedTugas = tugas; modalDetailTugas = true"
                                                ...
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    @if(in_array(($student->role ?? ''), ['ketua_kelas', 'super_admin']))
                                                        ...
                                                        <button
                                                            @click.stop="if(confirm('Hapus tugas/deadline ini?')) { fetch('/kh/assignment/'+tugas.id, {method: 'DELETE', headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}}).then(()=>location.reload()); }"
                                                            class="bg-red-600/20 border border-red-500/30 text-red-500 text-[8px] px-2 py-1 rounded font-bold hover:bg-red-600 hover:text-white transition ml-1 cursor-pointer">Hapus</button>
                                                    @endif
                                                </div>
                                            </div>
                                        </template>
                                                    ...
                                                    <template
                                                        ...
                                                        <div class="flex items-center justify-between p-2.5 rounded-xl bg-black/40 border border-zinc-800/50 hover:bg-zinc-800 transition group cursor-pointer"
                                                            ...
                                                            @if(in_array(($student->role ?? ''), ['ketua_kelas', 'super_admin']))
                                                                ...
                                                                <button
                                                                    @click.stop="if(confirm('Hapus modul ini?')) { fetch('/kh/module/'+modul.id, {method: 'DELETE', headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}}).then(()=>location.reload()); }"
                                                                    class="bg-red-600/20 border border-red-500/30 text-red-500 text-[8px] px-1.5 py-0.5 rounded font-bold hover:bg-red-600 hover:text-white transition ml-1 cursor-pointer">Hapus</button>
                                                            @endif
                            ...
                            <div class="flex flex-wrap gap-2 w-full md:w-auto">
                                @if(in_array($student->role ?? '', ['ketua_kelas', 'sekretaris', 'bendahara', 'super_admin']))
                                    ...
                                    <button @click="modalKas = true"
                                        class="flex-1 md:flex-none bg-white text-black font-bold px-5 py-2.5 rounded-xl hover:bg-zinc-200 shadow-lg justify-center transition flex items-center gap-2">
                                        Input Ekstra
                                    </button>
                                @endif
                                <button @click="modalPayKas = true"
                                    class="w-full md:w-auto bg-emerald-600 text-white font-bold px-6 py-2.5 rounded-xl hover:bg-emerald-500 justify-center shadow-lg transition flex items-center gap-2 mt-2 md:mt-0">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z">
                                        </path>
                                    </svg>
                                    Bayar Kas (Upload Resi)
                                </button>
                            </div>
                        </div>
                                        ...
                                        <template x-for="trx in semuaTransaksi" :key="trx.id">
                                            <tr class="hover:bg-zinc-800/20 transition">
                                                ...
                                                <td class="py-4 text-xs">
                                                    ...
                                                    <template x-if="trx.proof_image">
                                                        <div class="mt-2">
                                                            <button
                                                                @click.prevent="selectedProofImage = trx.proof_image; $dispatch('open-proof-modal')"
                                                                class="text-[9px] text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20 hover:bg-emerald-500/20 transition cursor-pointer">Lihat
                                                                Bukti Foto</button>
                                                        </div>
                                                    </template>
                                                </td>
                                                ...
                                                <template x-for="mhs in semuaMahasiswa" :key="mhs.id">
                                                    <tr class="hover:bg-white/5 transition group">
                                                        ...
                                                        <td class="px-4 py-3 text-right">
                                                            <button x-show="mhs.role !== 'super_admin'"
                                                                @click="let newRole = prompt('Ubah jabatan untuk ' + mhs.name + '?\n\nPilihan: mahasiswa, ketua_kelas, sekretaris, bendahara\n\nPENTING: Sistem akan otomatis mereset password ke NIM+InisialJabatan', mhs.role); if(newRole && ['mahasiswa', 'ketua_kelas', 'sekretaris', 'bendahara'].includes(newRole.trim()) && newRole.trim() !== mhs.role) { fetch('/kh/student/' + mhs.id + '/role', { method: 'PUT', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').getAttribute('content') }, body: JSON.stringify({ role: newRole.trim() }) }).then(res => res.json()).then(res => { if(res.success) { alert('Berhasil ubah jabatan!\n\nPassword login baru untuk ' + mhs.name + ' sekarang menjadi: ' + res.default_password); location.reload(); } else { alert('Gagal merubah data!'); } }).catch(e => alert('Terjadi kesalahan koneksi.')); } else if(newRole && newRole.trim() !== mhs.role) { alert('Format pilihan tidak sesuai! Harap ketik persis seperti teks pilihan kecil semua tanpa spasi ganda.'); }"
                                                                class="p-2 text-zinc-500 hover:text-blue-400 transition inline-flex items-center justify-center">
                                ...
                                <div class="bg-black/50 p-3 rounded-xl border border-zinc-800 flex justify-between items-center group cursor-pointer hover:bg-zinc-800 transition"
                                    ...
                                    <div class="flex items-center gap-2">
                                        @if(in_array(($student->role ?? ''), ['ketua_kelas', 'super_admin']))
                                            <button
                                                @click.stop="if(confirm('Hapus modul ini?')) { fetch('/kh/module/'+modul.id, {method: 'DELETE', headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}}).then(()=>location.reload()); }"
                                                class="text-red-500 hover:text-red-400 bg-red-500/10 hover:bg-red-500/20 px-2 py-1 rounded transition text-[10px] font-bold hidden group-hover:inline-block">Hapus</button>
                                        @endif
                                    ...
                                    <div class="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-black/50 p-4 rounded-xl border border-zinc-800 hover:border-zinc-700 cursor-pointer transition"
                                        ...
                                        <div class="flex items-center justify-between mb-1">
                                            ...
                                            <div class="flex items-center gap-2">
                                                @if(in_array(($student->role ?? ''), ['ketua_kelas', 'super_admin']))
                                                    <button
                                                        @click.stop="if(confirm('Hapus tugas ini?')) { fetch('/kh/assignment/'+tugas.id, {method: 'DELETE', headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}}).then(()=>location.reload()); }"
                                                        class="text-red-500 hover:text-red-400 bg-red-500/10 hover:bg-red-500/20 px-1.5 py-0.5 rounded transition text-[9px] font-bold hidden group-hover:block">Hapus</button>
                                                @endif
                                                <span
        ...
        <!-- Modal Edit Jabatan -->
        <div x-show="modalEditJabatan" x-transition.opacity
            class="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm"
            style="display: none;">
            <div @click.away="modalEditJabatan = false" x-show="modalEditJabatan"
                x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95"
                x-transition:enter-end="opacity-100 scale-100"
                class="bg-zinc-900 border border-zinc-800 rounded-3xl w-full max-w-sm p-6 space-y-5 shadow-2xl relative overflow-hidden">
                <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-500 to-indigo-500"></div>
                <div class="flex justify-between items-start">
                    <div>
                        <h3 class="text-lg font-bold text-white mb-1">Ubah Jabatan Pengurus</h3>
                        <p class="text-xs text-zinc-500">Data mahasiswa: <span class="font-bold text-zinc-300"
                                x-text="editJabatanTarget ? editJabatanTarget.name : ''"></span></p>
                    </div>
                    <button @click="modalEditJabatan = false"
                        class="text-zinc-500 hover:text-white bg-black p-2 rounded-full border border-zinc-800 transition">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                ...
                <form @submit.prevent="submitEditJabatan()" class="space-y-4">
                    <div class="relative">
                        <label class="block text-xs font-bold text-zinc-400 uppercase tracking-widest mb-2">Pilih
                            Jabatan <span class="text-blue-400">*</span></label>
                        <select x-model="editJabatanRole" required
                            class="w-full bg-black border border-zinc-800 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-blue-500 transition appearance-none">
                            <option value="mahasiswa">Mahasiswa Biasa</option>
                            <option value="ketua_kelas">Ketua Kelas</option>
                            <option value="sekretaris">Sekretaris</option>
                            <option value="bendahara">Bendahara</option>
                        ...
                        <div
                            class="pointer-events-none absolute inset-y-0 right-0 pt-[28px] flex items-center px-4 text-zinc-500">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3 flex gap-3">
                        ...
                        <p class="text-[10px] text-zinc-300 leading-relaxed">
                            <strong class="text-blue-400 block mb-0.5">PENTING: BACA TERLEBIH DAHULU</strong>
                            Mengubah jabatan akan mereset password mahasiswa. Jabatan pengurus menjadi <span
                                class="font-mono text-blue-300">NIM + InisialJabatan (KK/SK/BD)</span>, mahasiswa biasa
                            menjadi hanya <span class="font-mono text-zinc-300">NIM</span> Anda wajib langsung
                            memberitahu anggota yang bersangkutan.
                        </p>
                    </div>

                    <button type="submit"
                        class="w-full bg-blue-600 text-white py-3.5 rounded-2xl text-sm font-bold hover:bg-blue-500 transition shadow-lg shadow-blue-600/20">
                        Simpan Perubahan
                    </button>
                </form>
            </div>
        ...
        <div x-show="modalUploadQris" x-transition.opacity
            ...
            <div @click.away="modalUploadQris = false" x-show="modalUploadQris"
                ...
                <div>
                    ...
                    <p class="text-xs text-zinc-500">Unggah QRCode kas kelas (*.jpg / *.png).</p>
                </div>
